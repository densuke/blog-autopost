...
- UI本体はリポジトリルートの `static/index.html` と `static/login.html`（CSS/JSインライン）
  - 配信経路は `src/web/mod.rs` の `fallback_service(ServeDir::new("static").append_index_html_on_directories(true))`
  - ログイン画面は `src/web/routes.rs` が `static/login.html` を直接読み込む
  - `static/` は `just dist` の配布物にも含まれる
...
| `dark-mode-system-integration` | 完了 - システム連動に加え、手動切替UI(自動/ライト/ダーク)を実装 (#58) |
| `reserved-post-sns-selection-fix` | 完了 - 編集モーダルのSNS一覧を /api/config 由来へ変更し修正 (#57) |
| `test-coverage-improvement` | 完了 - カバレッジ80%達成(42.06% → 80.76%)。以降は閾値80での維持が目的 (#59 #61 #62 #63) |
| `mcp-server-enhancement` | 完了 - MCP専用APIキーの分離、セッション強化、メディアパス検証、toolギャップ解消、テストとドキュメント整備 (#92) |
...
- テストファーストとする
- テストは外部ネットワークへ接続しないこと
    - HTTPを伴う処理は `wiremock` でモックし、`MockServer::uri()` を注入する
    - テストデータのURLやドメインには RFC 2606 の予約ドメインを使うこと
      - `example.com` / `example.org` / `example.net` およびそのサブドメイン
      - 例: `https://mstdn.example.com`, `https://blog.example.com/feed.xml`
      - **実在するドメインを書かない**。`mstdn.jp` や `misskey.io` のような実際の
        サービス名を使うと、設定ミスで本当に接続してしまった場合に迷惑をかける
- テストは `data/` 配下の実ファイルを汚さないこと
    - ファイルを扱う処理は `tempfile` の一時ディレクトリを使う
...
- コードカバレッジは全体で80%以上の維持を目標とし、新規コードは90%以上を目指すこと
    ...
    - 当初の目標80%は到達済み。現在の閾値は89。以降は維持が目的であり、閾値を機械的に上げ続ける必要はない
      - 実測が閾値を5ポイント以上上回った状態が続くなら引き上げを検討する
    - CIが閾値割れで失敗したら、閾値を下げるのではなくテストを追加して対応する
      - どうしても難しい場合は、理由をPRに明記した上で相談すること
