...
def update_scheduled_post(
    ...
    if existing_post.status in ["実行済み", "失敗"]:
        raise HTTPException(
            status_code=409, detail="Cannot update an already executed or failed post"
        )
    ...
    if scheduled_at:
        ...
        updates["scheduled_at"] = (
            scheduled_at_tz if scheduled_at_tz is not None else scheduled_at
        )
    if content:
    ...
    if target_sns is not None:
        ...
        if not all(sns in supported_sns for sns in target_sns):
            raise HTTPException(
                status_code=400, detail="Unsupported SNS target specified"
            )
    ...
    if not updated_post:
        raise HTTPException(
            status_code=404, detail="Scheduled post not found after update attempt"
        )
    return updated_post
...
def delete_scheduled_post(
    ...
    if existing_post.status == "実行済み":
        raise HTTPException(
            status_code=409, detail="Cannot delete an already executed post"
        )

    if not store.delete_post(post_id):
        raise HTTPException(
            status_code=404, detail="Scheduled post not found for deletion"
        )
    return
...
def re_execute_scheduled_post(
    ...
    if existing_post.status == "実行済み":
        raise HTTPException(
            status_code=409, detail="Cannot re-execute an already successful post"
        )
    ...
    if not updated_post:
        raise HTTPException(
            status_code=404, detail="Scheduled post not found for re-execution"
        )
...
def send_scheduled_post_now(
    ...
    if existing_post.status == "実行済み":
        ...
        raise HTTPException(
            status_code=409, detail="Cannot send an already executed post immediately"
        )
    ...
    if not updated_post:
        raise HTTPException(
            status_code=404, detail="Scheduled post not found after immediate sending"
        )
...
def schedule_post_next_timing(
    ...
    for sns in target_sns:
        ...
        if slot_time is None:
            errors.append(
                {"sns": sns, "error": "7日以内に空きスロットが見つかりませんでした"}
            )
        ...
        try:
            ...
            errors.append({"sns": sns, "error": f"予約作成に失敗しました: {str(e)}"})
            logger.error(f"Failed to create post for {sns}: {e}")

    logger.info(
        f"User {user} created {len(created_posts)} posts with {len(errors)} errors"
    )

    return {"created_posts": created_posts, "errors": errors}
...
def api_schedule(
    ...
    try:
        ...
        return JSONResponse(
            content={"message": "Post scheduled successfully", "job_id": job_id}
        )
