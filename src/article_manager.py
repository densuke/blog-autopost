...
class ArticleManager:
    ...
    def _safe_load_json(self, file_path: str, default_value):
        """
    ...
    def force_mark_all_as_posted(self) -> dict:
        ...
        for feed_config in all_feed_configs:
            ...
            if not feed_url:
                print(
                    f"警告: フィード '{feed_name}' のURLが設定されていません。スキップします。"
                )
                continue
            ...
            if feed.bozo:
                print(
                    f"警告: フィード '{feed_name}' の解析に失敗しました: {feed.bozo_exception}"
                )
                continue
        ...
        # 既存の保存済み記事を読み込む
        saved_data = self._safe_load_json(DATA_FILE, {})
    ...
    def load_saved_articles(self):
        return self._safe_load_json(DATA_FILE, [])
    ...
    def get_new_articles(
        ...
        for article in new_articles:
            ...
            if "_feed_entry" in article:
                article["image"] = self._extract_article_image(
                    article["_feed_entry"], debug=debug
                )
                # フィード名を記事に追加
    ...
    def _extract_article_image(self, entry, debug=False):
        ...
        strategies = image_settings.get(
            "image_strategy", ["featured_image", "og_image", "default"]
        )
        ...
        for strategy in strategies:
            try:
                ...
                if image_url and self._is_valid_image(image_url, image_settings):
                    if debug:
                        print(
                            f"記事 '{entry.title[:30]}...' の画像を取得しました ({strategy}): {image_url}"
                        )
                    return image_url
    ...
    def _get_og_image(self, article_url, debug=False):
        ...
        try:
            ...
            response = requests.get(
                article_url,
                timeout=10,
                headers={"User-Agent": "Mozilla/5.0 (compatible; Blog-AutoPost/1.0)"},
            )
            response.raise_for_status()
            if debug:
                print(
                    f"[DEBUG] ページアクセス成功、HTMLサイズ: {len(response.text)} 文字"
                )
...
class MultiArticleManager:
    ...
    def load_saved_articles_for_feed(self, feed_name):
        ...
        if os.path.exists(feed_data_file):
            return self._safe_load_json(feed_data_file, [])

        # 従来の共通ファイルから該当フィードの記事を取得（移行用）
        all_articles = self._safe_load_json(DATA_FILE, [])
        if all_articles:
