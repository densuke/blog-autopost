...
def test_multi_article_manager_loads_legacy_common_file_dict(
    mock_config_manager, tmp_path
):
    """旧形式(JSONオブジェクト形式)の共通ファイルを安全に読み込める"""
    # --- Arrange ---
    multi_am = MultiArticleManager(mock_config_manager)
    import src.article_manager

    legacy_payload = {
        "__forced_posted_urls__": ["http://example.com/legacy"],
        "articles": [
            {"title": "legacy", "link": "http://example.com/a", "feed_name": "other"}
        ],
    }
    data_file_path = Path(src.article_manager.DATA_FILE)
    data_file_path.write_text(json.dumps(legacy_payload), encoding="utf-8")

    # --- Act ---
    saved_articles = multi_am.load_saved_articles_for_feed("tech-blog")
...
@patch("feedparser.parse")
def test_multi_article_manager_get_all_new_articles_success(
    mock_parse, mock_config_manager
):
    ...
    mock_tech_entry.published_parsed = datetime(
    ...
    mock_news_entry.published_parsed = datetime(
...
@patch("feedparser.parse")
def test_multi_article_manager_get_all_new_articles_with_feed_filter(
    mock_parse, mock_config_manager
):
...
@patch("feedparser.parse")
def test_multi_article_manager_get_all_new_articles_no_articles(
    mock_parse, mock_config_manager
):
...
def test_article_manager_get_new_articles_with_limit(mock_parse, mock_config_manager):
    ...
    for i in range(10):
        ...
        entry.published_parsed = datetime(
            2024, 1, 1, 10, 0, 0, tzinfo=timezone.utc
        ).timetuple()
        entry.enclosures = []
        entry.media_content = []
...
@patch("feedparser.parse")
def test_article_manager_create_post_text_with_url_shortening(
    mock_parse, mock_config_manager
):
...
@patch("feedparser.parse")
def test_article_manager_create_post_text_different_sns(
    mock_parse, mock_config_manager
):
...
@patch("feedparser.parse")
def test_article_manager_force_mark_all_as_posted_success(
    mock_parse, mock_config_manager, tmp_path
):
...
def test_article_manager_get_new_articles_empty_saved(mock_parse, mock_config_manager):
    ...
    mock_entry.published_parsed = datetime(
        2024, 1, 1, 10, 0, 0, tzinfo=timezone.utc
    ).timetuple()
...
@patch("feedparser.parse")
def test_article_manager_parse_published_date_invalid_format(
    mock_parse, mock_config_manager
):
...
@patch("feedparser.parse")
def test_article_manager_is_valid_image_with_dimensions(
    mock_parse, mock_config_manager
):
    ...
    image_settings = {"image_filters": {"min_width": 200, "min_height": 200}}
...
@patch("feedparser.parse")
def test_multi_article_manager_get_all_new_articles_with_debug(
    mock_parse, mock_config_manager, capsys
):
...
@patch("feedparser.parse")
def test_article_manager_get_latest_articles_with_debug(
    mock_parse, mock_config_manager, capsys
):
