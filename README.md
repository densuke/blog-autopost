...
Blog AutoPost CLIは、各SNSの投稿タイミングを細かく制御できる機能を備えています。
デフォルトのタイミング設定と、SNS毎の個別設定により、最適な時間帯に投稿できます。

#### 機能概要

- **スマートスケジューリング**: 「次のタイミングで投稿」機能で、各SNSの次の空きスロットに自動予約
- **デフォルト設定**: 全SNS共通の投稿可能時間帯を指定
- **SNS別カスタマイズ**: 各SNSごとに異なる投稿時間帯を設定可能
- **ワイルドカード対応**: 曜日指定（`Weekday`, `Weekend`）で複雑なスケジュール実現
- **トレランス**: 指定時刻の前後で投稿時間を調整（デフォルト: 5分）
...
default_allowed_timings:
  - ["*", ["09:00", "12:00", "15:00", "18:00", "21:00"]]  # 毎日5つの時間帯で投稿可能

# 投稿時刻のトレランス（分単位、デフォルト: 5）
allowed_timings_tolerance_minutes: 5
```

#### ワイルドカード拡張

曜日指定には以下のワイルドカード形式が使用できます：

- **`"*"`**: 毎日（デフォルト）
- **`"Weekday"`**: 平日（月～金）のみ
- **`"Weekend"`**: 土日のみ
- **曜日名**: `"Monday"`, `"Tuesday"`, `"Wednesday"`, `"Thursday"`, `"Friday"`, `"Saturday"`, `"Sunday"`

#### SNS別タイミング設定

各SNSに異なるタイミングを設定できます：

```yaml
default_allowed_timings:
  - ["*", ["09:00", "15:00", "21:00"]]  # 全SNS共通: 朝9時、午後3時、夜9時

# SNS別の個別設定
allowed_timings:
  x:
    - ["Weekday", ["08:00", "12:00", "17:00", "20:00"]]    # 平日は朝8時、12時、17時、20時
    - ["Weekend", ["10:00", "14:00", "18:00"]]              # 土日は10時、14時、18時
  
  bluesky:
    - ["*", ["07:00", "13:00", "19:00"]]                    # 毎日7時、13時、19時
  
  mastodon:
    - ["*", ["06:00", "12:00", "18:00", "22:00"]]           # 毎日6時、12時、18時、22時
  
  misskey:
    - ["Weekday", ["09:00", "15:00", "21:00"]]              # 平日のみ
    - ["Saturday", ["11:00", "16:00", "20:00"]]             # 土曜のみ
    - ["Sunday", ["10:00", "15:00", "19:00"]]               # 日曜のみ
```

#### 時刻トレランス設定

投稿時刻がちょうど指定時間でなくても、トレランス内なら投稿できます：
...
例: `09:00` に投稿予約した場合、`08:50` ～ `09:10` の間なら投稿実行されます。
...
```yaml
default_allowed_timings:
  - ["*", ["09:00", "18:00"]]  # 全SNス共通のフォールバック

allowed_timings:
  x:
    # ビジネスアワーの活用が多いX
    - ["Weekday", ["08:00", "09:00", "12:00", "13:00", "18:00", "19:00"]]
    - ["Weekend", ["10:00", "14:00", "20:00"]]
  
  bluesky:
    # 朝活ユーザーが多いBluesky
    - ["*", ["06:00", "07:00", "12:00", "20:00", "21:00"]]
...
ダッシュボードの「次のタイミングで投稿」機能を使用することで、以下の流れで投稿できます：

1. 投稿内容とメディアを入力
2. 対象SNSを選択
3. 「次のタイミングで投稿」ボタンをクリック
4. システムが各SNS毎に次の空きスロットを自動検索
5. 各SNSの予約投稿一覧に追加

スロット検索は7日以内の範囲で行われます。

#### 注意事項

- タイムゾーンはサーバーのローカルタイムゾーンが使用されます
- 設定ファイルの `allowed_timings_tolerance_minutes` のデフォルト値は5分です
- SNS別設定がない場合は `default_allowed_timings` が使用されます
- スロット検索範囲は7日以内です（見つからない場合はエラーになります）
