...
**検証方法**:
- [x] ブランチが `feature/unified-post-form-ui` に切り替わった
- [x] `git status` でクリーンな状態確認
- [x] `git log` で最新のmainコミットが確認できる
...
**検証方法**:
- [x] 日時ピッカーが `id="scheduled-controls-area"` でラップされている
- [x] スロット検索説明が `id="next-timing-info-area"` でラップされている
- [x] ボタンが1つに統合されている
- [x] 初期状態で日時ピッカーと説明が非表示
...
**検証方法**:
- [x] ラジオボタンが視認可能なスタイルで表示
- [x] 選択状態が視覚的に区別される
- [x] 条件付き表示がスムーズなトランジション
- [x] レイアウトシフトが発生しない
...
**検証方法**:
- [x] `postState` オブジェクトが作成されている
- [x] `init()` メソッドがLocalStorageから値を読み込める
- [x] `setMethod()` がLocalStorageに保存できる
- [x] ページリロード後も値が保持される
...
**検証方法**:
- [x] ラジオボタン選択時にイベントが発火
- [x] `postState.method` が更新される
- [x] LocalStorageに保存される
- [x] ページリロード後にラジオボタンが復元される
- [x] コンソールに適切なログが出力される
...
**検証方法**:
- [x] 「すぐに投稿」選択時、両方の条件付きエリアが非表示
- [x] 「時間指定」選択時、日時ピッカーが表示、説明は非表示
- [x] 「次のタイミング」選択時、説明が表示、日時ピッカーは非表示
- [x] ボタンラベルが正しく変更される
- [x] トランジションがスムーズ
- [x] レイアウトシフトなし
...
**検証方法**:
- [x] 「すぐに投稿」選択 → `/api/post` が呼ばれている
- [x] 「時間指定」選択 → `/api/scheduled-posts` が呼ばれている
- [x] 「次のタイミング」選択 → `/api/scheduled-posts/next` が呼ばれている
...
**検証方法**:
- [x] ネットワークリクエストが正しいエンドポイントに送信されている
- [x] ステータスコード 200/201 が返されている
- [x] レスポンスデータが正しい形式
- [x] 予約一覧が更新されている
- [x] トーストメッセージが表示される
...
**検証方法**:
- [x] ネットワークエラー時にメッセージが表示される
- [x] APIエラー時にメッセージが表示される
- [x] スロット検索失敗時にメッセージが表示される
- [x] フォームがリセットされない（再入力可能）
- [x] ボタンが無効化されない（再試行可能）
...
**検証方法**:
- [x] 各ブレークポイントでレイアウトが正常
- [x] キーボードナビゲーション可能
- [x] スクリーンリーダー対応
- [x] 視認性に問題なし
- [x] トランジション時にレイアウトシフトなし
...
- [x] **パフォーマンス**: UI更新 100ms以内
- [x] **レスポンシブ**: 3ブレークポイント全対応
- [x] **ブラウザ互換**: Chrome/Firefox/Safari/Edge の最新版で動作
